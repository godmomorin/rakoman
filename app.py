...
TEMPLATES = {'404.html': '{% extends "base.html" %}\n'
 ...
 'base.html': '<!DOCTYPE html>\n'
              ...
              '  transition:transform .12s;display:block;\n'
              '}\n'
 ...
 'index.html': '{% extends "base.html" %}\n'
               ...
               '    <div class="body">\n'
               '      {% if p.genre %}\n'
               '      <a href="{{ url_for(\'index\', genre=p.genre) }}" onclick="event.stopPropagation()"\n'
               '         style="display:block;text-align:center;border-radius:10px;padding:3px 6px;margin-bottom:3px;font-size:.8rem;font-weight:700;color:#fff;background:{{ '
               'GENRE_COLOR.get(p.genre,\'#7a7484\') }};border:2px solid var(--ink);text-decoration:none">{{ p.genre }}</a>\n'
               '      {% endif %}\n'
               ...
               '      <div class="user-link" data-user="{{ p.username }}" style="display:flex;align-items:center;gap:4px;font-size:.78rem;color:#7a7484;cursor:pointer">\n'
               '        {% if p.user_icon %}<img class="mini-icon" src="{{ url_for(\'media\', filename=p.user_icon) }}" alt="">{% else %}😀{% endif %}\n'
               '        <u>{{ p.username }}</u>\n'
               '      </div>\n'
               '      <div style="display:flex;gap:6px;font-size:.76rem;font-weight:700;flex-wrap:wrap;margin-top:1px">\n'
               '        <span>▶️ {{ p.views }}</span><span>👍 {{ p.likes }}</span><span>🤩 {{ p.fav_count }}</span><span>💬 {{ p.comment_count }}</span>\n'
               '      </div>\n'
               '    </div>\n'
               '  </a>\n'
               '  {% endfor %}\n'
