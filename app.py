...
TEMPLATES = {'404.html': '{% extends "base.html" %}\n'
 ...
 'base.html': '<!DOCTYPE html>\n'
              ...
              '  background:linear-gradient(135deg,var(--soft),#fffbe9);font-size:2.2rem;\n'
              '  border-bottom:2.5px solid var(--ink);flex-shrink:0;\n'
              '}\n'
              '.card .thumb img{width:100%;height:100%;object-fit:cover;display:block}\n'
              '.card .body{padding:8px 12px 10px;display:flex;flex-direction:column;gap:3px;flex:1}\n'
 ...
 'index.html': '{% extends "base.html" %}\n'
               ...
               'genre|urlencode }}{% endif %}" style="display:flex;flex-direction:column">\n'
               '    <div '
               'style="width:100%;aspect-ratio:16/10;overflow:hidden;background:linear-gradient(135deg,#fff6f0,#fffbe9);display:flex;align-items:center;justify-content:center;font-size:2.2rem;border-bottom:2.5px '
               'solid var(--ink);flex-shrink:0">\n'
               '      {% if p.thumb %}<img src="{{ url_for(\'media\', filename=p.thumb) }}" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;display:block">{% else %}🎙️{% endif '
               '%}\n'
               '    </div>\n'
               '    <div style="padding:8px 10px 10px;display:flex;flex-direction:column;gap:3px;flex:1">\n'
               '      {% if p.genre %}<a style="background:{{ GENRE_COLOR.get(p.genre,\'#7a7484\') }};display:block;text-align:center;border-radius:10px;padding:3px '
               '6px;margin-bottom:3px;font-size:.8rem;font-weight:700;color:#fff;border:2px solid var(--ink)"\n'
               '         href="{{ url_for(\'index\', genre=p.genre) }}" onclick="event.stopPropagation()">{{ p.genre }}</a>{% endif %}\n'
               '      <div style="font-weight:900;font-size:.92rem;line-height:1.35">{{ p.title }}{% if not p.is_public %} <span class="badge-private">非公開</span>{% endif %}</div>\n'
               '      <div class="user-link" data-user="{{ p.username }}" style="display:flex;align-items:center;gap:5px;font-size:.78rem;color:#7a7484;cursor:pointer">\n'
               '        {% if p.user_icon %}<img class="mini-icon" src="{{ url_for(\'media\', filename=p.user_icon) }}" alt="">{% else %}😀{% endif %}\n'
               '        <u>{{ p.username }}</u>\n'
               '      </div>\n'
               '      <div style="display:flex;gap:7px;font-size:.78rem;font-weight:700;flex-wrap:wrap"><span>▶️ {{ p.views }}</span><span>👍 {{ p.likes }}</span><span>🤩 {{ p.fav_count '
               '}}</span><span>💬 {{ p.comment_count }}</span></div>\n'
               '    </div>\n'
               '  </a>\n'
               '  {% endfor %}\n'
               '</div>\n'
 ...
 'profile.html': '{% extends "base.html" %}\n'
                 ...
                 '}};display:block;text-align:center;border-radius:12px;padding:3px 8px;margin-bottom:4px;font-size:.82rem" onclick="event.stopPropagation()">{{ p.genre }}</a>{% endif %}\n'
